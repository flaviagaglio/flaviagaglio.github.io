...
export const projects: Project[] = [
  ...
  },
  {
    ...
    category: 'audio',
    summary:
      "A real, playable synthesizer controlled entirely by hand gestures over webcam — no install, no MIDI controller. Right hand plays scale-quantized melody by pinch and position; left hand shapes filter and resonance; reverb, delay and chorus run as independent sliders. The background grid doubles as a pitch guide, marking exactly where each note lives before you ever raise a hand.",
    ...
    metrics: [
      ...
      { label: 'Scales', value: '6, always in-tune' },
      { label: 'Effects', value: '3 independent sends' },
    ],
    body: {
      problem:
        "Webcam hand-tracking demos are common; instruments you'd actually want to keep playing are not. The hard part isn't detecting a hand, it's turning noisy, frame-to-frame landmark jitter into something that sounds intentional, keeping every control legible enough that a first-time player isn't guessing where to move their hand, and making a mapping simple enough that the first gesture someone tries already sounds musical, with no way to hit a wrong note.",
      approach:
        "Right hand: vertical position is quantized live onto a musical scale (Minor/Major Pentatonic, natural Major/Minor, Dorian, or Hijaz) across one octave, so pitch is always in-key; a thumb-index pinch triggers the note, and how tightly you pinch sets its velocity. Left hand: position drives filter cutoff (mapped exponentially, since pitch/cutoff perception is logarithmic) and resonance, and a closed fist mutes everything for pauses. Reverb, delay and chorus are deliberately manual sliders rather than gestures — three effects with genuinely different characters couldn't share one hand-tracked knob without feeling like a compromise. The background grid's lines sit exactly at each scale degree's on-screen height, labeled on both edges, so pitch is something you can see and aim for rather than search for by ear. Every continuous signal passes through one-pole exponential smoothing plus a freeze-then-decay fallback before it touches an audio parameter, so a hand briefly leaving frame glides to a safe default instead of clicking or jumping.",
      architecture:
        "MediaPipe's HandLandmarker runs client-side against the raw (unmirrored) camera feed while the video displays CSS-mirrored for a natural selfie view — correcting MediaPipe's handedness labels for that mismatch is the single most common source of bugs in this kind of project, so it's handled explicitly and deliberately, with a same-hand fallback when only one hand is visible so audibility never hinges on getting that label exactly right. Audio runs on Tone.js for scheduling and effects, plus two hand-written AudioWorkletProcessors — a Karplus-Strong plucked-string model (a noise burst decaying through a damped delay loop) and a wavetable oscillator morphing between procedurally generated single-cycle frames — for the two engines Tone's built-ins can't do alone, routed through Tone's own createAudioWorkletNode helper rather than the raw Web Audio constructor (Tone's context isn't a native BaseAudioContext, which the browser's AudioWorkletNode silently rejects otherwise). Reverb, delay and chorus connect as three parallel sends off the filtered signal rather than one combined wet/dry blend, each independently controllable.",
      results:
        'A genuinely playable instrument where the default patch already sounds intentional with no tweaking — including on mobile Safari, where starting the AudioContext before awaiting the camera permission prompt (not after) turned out to be the fix for audio staying silently locked. Verified end-to-end with headless-browser smoke tests (fake camera and audio devices) across multiple iterations, which caught and fixed several real bugs before and during ship: AudioWorklet files silently failing to bundle, HUD text rendering mirrored, and engine switches silently failing and falling back to the previous engine.',
      limits:
  ...
  },
  {
    ...
    category: 'systems',
    summary: 'A real client-server Tic Tac Toe: a Cloudflare Durable Object owns the board and decides every move, over a live WebSocket connection.',
    stack: ['TypeScript', 'Cloudflare Workers', 'Durable Objects', 'WebSockets', 'Vite'],
    status: 'live-demo',
    featured: false,
    metrics: [{ label: 'Type', value: 'Web tool' }],
    body: {
      problem: 'A simple game is a useful, low-risk vehicle for practicing clean client-server design end to end — but a design only actually proves anything once it is live and reachable, not just built and marked done.',
      approach: 'Rebuilt an earlier private coursework version (Java, sockets) as a real, playable web app: a Cloudflare Durable Object is the sole authority on game state — board, whose turn it is, legality of a move, who won — and two clients talk to it over WebSockets. Share a room link for a real two-player game, or play instantly against the server\'s own minimax AI when there\'s no one else to play with.',
      architecture: 'Client-server, not peer-to-peer: clients never decide the outcome of a move, they send an attempt and render whatever state the server sends back. The server is a Cloudflare Worker + Durable Object (one object per room, addressed by room ID) written in TypeScript; the frontend is a small dependency-free Vite + TypeScript app deployed separately to GitHub Pages.',
      results: 'A genuinely networked two-player game — verified by opening two independent browser sessions and watching moves propagate over the real WebSocket connection, not simulated locally — plus a perfect-play (never loses) minimax opponent for solo play, both served by the same authoritative game loop.',
      limits: 'A disconnect mid-game does not preserve the seat — refreshing rejoins the room and takes whichever seat is open. Scoped as a demonstration of a real client-server exchange, not a production matchmaking service.',
    },
    links: { demo: 'https://flaviagaglio.github.io/tic-tac-toe/', code: 'https://github.com/flaviagaglio/tic-tac-toe' },
  },
  {
  ...
  },
  {
    ...
    title: 'Mode Finder',
    category: 'music',
    summary: 'The seven modes of the major scale built on any root note, with theory-correct note spelling — no repeated or skipped letter names.',
    stack: ['JavaScript', 'Music Theory'],
    status: 'live-demo',
    featured: false,
    metrics: [{ label: 'Type', value: 'Web tool' }],
    body: {
      problem: 'Working out all seven modes on a given tonic by ear or from memory is slow, and naive implementations get the note spelling wrong — reusing a single sharps-or-flats table for every note produces scales that repeat or skip letter names instead of real 7-note scales.',
      approach: 'Pick a root, get all seven modes (Ionian through Locrian) instantly, each spelled correctly: step through the musical alphabet one letter per degree and derive the accidental each degree needs from the mode\'s interval pattern, the same method notation software uses.',
      architecture: 'Single-page vanilla JavaScript. For the five enharmonically ambiguous roots (e.g. C♯/D♭), each mode independently picks whichever letter spelling needs fewer accidentals, so the same root can show D♭ Ionian next to C♯ Dorian in one result set.',
      results: 'Verified against 24 standard reference scales (all 12 major keys and 12 natural minor keys) with zero mismatches, and exhaustively checked across all 12 roots × 7 modes (84 combinations): every one uses each letter name exactly once with no double accidentals.',
      limits: 'Covers the seven modes of the major scale on any tonic; doesn’t extend to melodic/harmonic minor modes or other non-diatonic scales.',
    },
    links: { demo: '/mode-finder/' },
  },
  {
    slug: 'voicings',
    title: 'Voicings — Chord Diagrams for Piano & Guitar',
    category: 'music',
    summary: 'Type or pick any chord and see how to actually play it: a labeled piano keyboard plus three genuinely different guitar voicings spread across the neck.',
    stack: ['React', 'TypeScript', 'Tailwind CSS'],
    status: 'live-demo',
    featured: false,
    metrics: [{ label: 'Type', value: 'Web tool' }],
    body: {
      problem: 'Knowing that "Cm7b5" is a valid chord symbol doesn\'t tell you where to put your fingers — turning a chord name into an actual playable shape takes either years of practice or a lookup, and most chord-chart tools only cover a handful of qualities or reuse the same guitar shape at different frets and call it three voicings.',
      approach: 'Type or pick any of 25 chord qualities (triads through extended and altered) on any root and get a labeled piano keyboard plus three real, differently-positioned guitar voicings: authentic open-position and movable barre shapes for the most common chord types, and an algorithmic fretboard search for everything else.',
      architecture: 'React, TypeScript and Tailwind CSS, all SVG for the diagrams. A pure-TypeScript music theory engine spells every chord tone as a real letter-stepped interval from the root rather than just a pitch class, and a separate voicing engine searches fretboard windows for fingerings that stay within a playable span and actually contain the chord\'s defining tones.',
      results: 'Verified independently against 33 hand-written reference chord spellings and exhaustively across all 12 roots × 25 qualities (300 combinations) with zero mismatches, including all 20 classic open-position chords matching real, standard fingerings exactly; every guitar voicing shown is checked to use only real chord tones with the root always present.',
      limits: 'Guitar voicings for the qualities beyond the CAGED-based shapes are computed rather than hand-curated, so they\'re correct but not always the exact shape a working guitarist would reach for first.',
    },
    links: { demo: 'https://flaviagaglio.github.io/voicings/', code: 'https://github.com/flaviagaglio/voicings' },
  },
  {
    slug: 'passwords',
    title: 'Passwords — Password Generator',
    category: 'security',
    summary: 'Password generator styled as a flat hardware control panel — adjustable length and character types, strength meter driven by real entropy.',
    ...
    body: {
      ...
      approach: 'A generator with adjustable length and character-set options and a live strength meter, styled as a flat hardware control panel with an LCD-style readout for the generated password.',
      architecture: 'Self-contained vanilla JavaScript single page. Randomness comes from the Web Crypto API (crypto.getRandomValues) with rejection sampling to avoid modulo bias — a cryptographically secure source, not Math.random(). The strength meter shows actual Shannon entropy in bits (length × log2(alphabet size)), not an arbitrary score.',
      results: 'A tool actually used to generate passwords, with a strength indicator that reflects real entropy rather than a made-up heuristic.',
      limits: 'Client-side only — a generator is only as trustworthy as the machine and browser running it; it cannot protect against a compromised device.',
    },
